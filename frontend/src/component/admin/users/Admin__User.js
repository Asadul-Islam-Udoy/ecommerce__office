...
import { Admin_User_Reset, GetAllUsersAction, PermissionUsersAction } from '../../../actions/UsersAction';
import { useAlert } from 'react-alert';
function Admin__User() {
    const dispatch = useDispatch();
    const alert = useAlert()
    const {userInfo} = useSelector(state=>state.UserLoginReducer)
    const {lodding,error,isPermission,allUsers} = useSelector(state=>state.getAllUserStore)
    useEffect(()=>{
      if(error){
        alert.error(error)
      }
        ...
        },
         {
          field: 'avatar',
            ...
            renderCell:(params)=>{
                return(
                    <>
                    <img src={`http://localhost:5000/images/avatar/${params.row.avatar}`}/>
                    </>
                )
            }
          },
                    ...
                    <div>
                        <select className='permission__section'>
                            <option>Permissin Role</option>
                             {roles.map((item)=>(
                                <option onClick={()=>permissionHandler(userInfo?.user?._id,params.row.id,item)} value={item}>{item}</option>
                             ))}
                        </select>
