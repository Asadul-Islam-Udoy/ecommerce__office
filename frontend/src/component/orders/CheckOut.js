...
import { AddToCardAction, DeleteCartItemAction } from '../../actions/OrderAction';
function CheckOut() {
    const{id} = useParams()
    ...
    <div style={{marginTop:'-10px'}}>
     ...
     { cartItems?.length > 0 ?
      <div  className='checkout__container'>
        ...
        <div className='checkout__box__1'>
          ...
          <div>
          <div>
              <img src={`http://localhost:5000/images/products/${item.productImage}`} alt='images'/>
              <p>{item.name}</p>
           ...
           <div>
              <div title='delete' className='checkout__delete__product' onClick={()=>deleteCartHandler(item.product)}><HighlightOffIcon/></div>
              <div>
              <div className='quantity__section__checkout'>
                    <button onClick={()=>quantityDecrementHandler(item.product,item.quantity,item.colors,item.sizes)}>-</button>
                    <div>
                    ...
                    <button onClick={()=>quantityIncrementHandler(item.product,item.quantity,item.colors,item.sizes,item.stock)}>+</button>
                  </div>
              </div>
           </div>
     ...
     <div className='no__cartItem_section'>
        <div>
            ...
            <p>No Card Items</p>
            <Link to='/all/products'><button>View Products</button></Link>
        </div>
     </div>
     }
