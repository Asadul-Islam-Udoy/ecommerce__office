...
function SingleProduct() {
  ...
  const{error,lodding,product} = useSelector(state=>state.singleProduct)
     ...
     <div>{review && <ReviewCreate productId={product?._id}/>}</div>
      <div className='single__product__container'>
         <div style={{marginTop:'40px'}} className='single__product__header'>
           <img src='https://cdn.shopify.com/shopifycloud/hatchful_web_two/bundles/11e0719e75e67bbeafcfce50b1b0ceb4.svg' alt='imsfer'/>
             <div>
           ...
           <div className='single__product__box'>
              ...
              {product?.productImages?.slice(counts-1,counts).map((item)=>(
                <div className='zoom__images__section'>
                 ...
                 <div className='pariend__image'>
                    {product?.productImages?.map((item,index)=>(
                     <div>
                        <img onClick={()=>setCounts(index+1)}src={`http://localhost:5000/images/products/${item.image}`} alt={item._id}/>
                     </div>
                    ))}
               ...
               <div>
                  <div>
                   <h3>{product?.name}</h3>
                   <span><Rating  name="read-only" value={value} readOnly /></span>
                    <p style={{marginTop:'-5px'}}>{product?.ratings?.toFixed(1)} rating</p>
                    <hr/>
                  </div>
                      ...
                      <p>Size</p> 
                       <div>
                        {product?.sizes?.map((item,index)=>(
                          <span>{item}
                            <Checkbox onClick={()=>sizeHandler(item)} {...label} defaultChecked ={false} 
                            style ={{
                            color:`${item}`}} 
                            />
                            </span>
                      ...
                      <p>Colors</p> 
                       <div>
                         {product?.colors?.map((item,index)=>(
                          <span>{item}
                            <Checkbox onClick={()=>colorHandler(item)} {...label} defaultChecked ={false} 
                            style ={{
                            color:`${item}`}} 
                            />
                            </span>
                 ...
                 <div>
                   <p style={{fontSize:'14px',fontStyle:'italic'}}>{product?.description}</p>
                </div>
              </div>
                         ...
                         <div className='quantity__overflow'>
                           <div>
                            {[...Array(product?.stock).keys()].map((i)=>(
                              <span onClick={()=>setQuantity(i+1)} style={{backgroundColor:`${i+1===quantiry?'tomato':''}`}}>{i+1}</span>
                            ))}
                           </div>
                        </div>
                    </div>
                ...
                <div className='review__role__section'>
                  <p>Reviews role</p>
                    <div className='review__box'>
                      <div>
                        <span style={{color:'red',fontSize:'10px'}}>5 start</span>
                       <div> <p style={{width:`${Five}%`}}></p></div>
                        <span style={{color:'red',fontSize:'12px'}}>{Five}%</span>
                      </div>
                      <div>
                        <span style={{color:'red',fontSize:'10px'}}>4 start</span>
                       <div> <p style={{width:`${Four}%`}}></p></div>
                        <span style={{color:'red',fontSize:'12px'}}>{Four}%</span>
                      </div>
                      <div>
                        <span style={{color:'red',fontSize:'10px'}}>3 start</span>
                       <div> <p style={{width:`${Thire}%`}}></p></div>
                        <span style={{color:'red',fontSize:'12px'}}>{Thire}%</span>
                      </div>
                      <div>
                        <span style={{color:'red',fontSize:'10px'}}>2 start</span>
                       <div> <p style={{width:`${Two}%`}}></p></div>
                        <span style={{color:'red',fontSize:'12px'}}>{Two}%</span>
                      </div>
                      <div>
                        <span style={{color:'red',fontSize:'10px'}}>1 start</span>
                        <div><p style={{width:`${One}%`}}></p></div>
      ...
      <div className='categori__basic'>
        ...
        <CategoriBasic  categoryId={product?.category} productId={product?._id} slug={slug}/>
      </div>
      <div className='categori__basic'>
