...
function CategoriBasic({categoryId,productId,slug}) {
    ...
    const{lodding,error,singleProducts} = useSelector(state=>state.singleCategoryProducts)
           ...
           {singleProducts?.slice(count_button,count_button+4).map((item,index)=>(
                <div className='category__box'>
                    <Link to={`/category/single/${slug}/${item._id}`}>
                     ...
                     <div>
                      <span style={{marginTop:'-10px'}}><Rating  name="read-only" value={item.ratings} readOnly /></span>
                      <span>(12{index})</span>
                     </div>
                   </div>
                 ))}
               <div className='category__button'>
                 <p className='decrement' onClick={decrementFun}><FirstPageIcon/></p>
                 <p className='increment'  onClick={incrementFun}><LastPageIcon/></p>
               </div>  
           </div>
           :
