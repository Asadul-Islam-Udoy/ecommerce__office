...
function AllProducts() {
  ...
  const{lodding,error,products} = useSelector(state=>state.getAllProducts)
  const[pagination,setPagination] = useState(1)
  const[nextP,setNextP] = useState(0);
  const[preP,setPreP] = useState(0)
  const[counts,setCounts] = useState(0)
  const [initionCount,setInitionCount] = useState(6);
  ...
  const nextHandler=()=>{
    ...
    if(initionCount === pagination){
        setInitionCount( initionCount + 6);
        setCounts(counts+3)
    }
  }
  const preHandler=()=>{
...
useEffect(()=>{
  ...
  return (
    ...
    {lodding && <Lodder/>}
     <div ><Navber/></div>
     <div className='paroduct__category__container' style={{minHeight:'80vh', paddingTop:'70px'}}>
      ...
      <div className='product__category__section'>
        {products?.slice(pagination*6-6,pagination*6).map((item)=>(
            <div>
            <div className='product__category__box'>
             <Link to={`/category/single/${slug}/${item._id}`}><img src={`http://localhost:5000/images/products/${item.productImages[0].image}`}/></Link>
              <div>
                  <p>{item.name}</p>
                  <span><Rating  name="read-only" value={item.ratings} readOnly size='large'/></span>
                 <div className='category__price'>
                 <p className='price__section'><b>${item.price}</b></p>
                 <p>{item.offer}%</p>
                 </div>
              </div>
            </div>
      ...
      <div className='pagination__section'>
        {pagination !== 0 &&  <span onClick={preHandler}><ArrowLeftIcon style={{fontSize:'16px'}}/>pre</span> }
        {arrayList.slice(counts,initionCount).map((item)=>(
           <span style={{backgroundColor:`${item===pagination?'gray':''}`}} onClick={()=>panginationHandler(item)}>{item}</span>
        ))}
       {pagination !== arrayList.length &&  <span onClick={nextHandler}>next<ArrowRightIcon style={{fontSize:'16px'}}/></span>}
       </div>
       <div>
        <Footer/>
       </div>
