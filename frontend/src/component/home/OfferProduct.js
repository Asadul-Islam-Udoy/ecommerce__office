...
function OfferProduct() {
   const {lodding,error,products} = useSelector(state=>state.homeUniqueProductStore)
                 ...
                 {item.offer > 0 && <div className='offer__persen__section'><p>{item.offer}%</p></div>}
                  ...
                  <div className='name__section'>
                     <div><span>{item.name}</span></div>
                      <br/>
                      <div><span>$ {item.price}</span><span>${item.oldPrice}</span></div>
                      <br/>
                     <div>  <span><Rating style={{marginTop:'-10px'}}  name="read-only" value={item.ratings} readOnly /></span><span style={{marginTop:'15px'}}>{item.numOfReview}( reviews)</span></div>
                   </div>
              </div>
              </Link>
           ))}
